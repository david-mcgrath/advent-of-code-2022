...
function getSplitSets(arr) {
	let result = [[]];
	
	for (let i = 0; i < arr.length; i++) {
		result = result.concat(result.map(x => x.concat(arr[i])));
	}
	
	return result
		.map(x => [x, arr.filter(y => x.indexOf(y) < 0)]);
}
...
await getInput(isTest)
	.then(input => {
		...
		let splits = getSplitSets(nonZeroValves);
		
		let max = 0;
		
		function getMaxFlow(nonZeroValves) {
			let stack = [{
				location: start,
				timeRemaining: 26,
				flowRate: 0,
		...
		for (let i = 0; i < splits.length; i++) {
			...
			let flow = getMaxFlow(split[0]) + getMaxFlow(split[1]);
